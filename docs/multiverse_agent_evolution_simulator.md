...
## 12. 확정 구현 스펙 (1차) — 학습 정리 반영
아래는 개념 학습(record/*.md)을 통해 확정한 1차 구현 결정 사항입니다. 세부 유도 과정은 각 학습 기록 참고. 원칙은 일관되게 **"단순한 것부터, 변수 하나씩 추가, 필요 확인 후 복잡도 상승"**입니다.

### 12.1 성격 벡터 (Persona)
- **1차: Big 5(OCEAN) 5차원** — `[개방성, 성실성, 외향성, 우호성, 신경증]`. 값 범위 내부 `0~1`, 표시 시 ×100(%).
- 2차: 결과가 괜찮으면 TCI 추가 → PCA로 차원 축소. 성격 축 간 상관관계(신경증↑→성실성↓ 등)도 2차.
- 1차에서는 **각 축 독립**(상관관계 미적용) — 원인 분리(디버깅) 용이.

### 12.2 시간 / tick
- **tick = 한 달(추상 단위).** 1년 = 12 tick, 100년 = 1,200 tick. 실제 달력 일수(2월 28/29)는 표현 안 함.
- `T_end = 1,000` → 약 83년 생애. 하루 단위(25,550 tick) 대비 25배 경량, 월별 이벤트는 표현 가능.
- "미세한 변화"는 tick 길이가 아니라 **변화량($f$)**이 결정.

### 12.3 상태 전이 규칙 $f$ (한 tick)
```
1. 이벤트 뽑기(시드·확률)
2. 성격 업데이트 — 이벤트가 건드리는 각 축에 적응(adaptation) 공식 독립 적용:
      distance = (Δ > 0) ? (1 - n) : n
      n        = n + Δ × distance          # 경계(0/1)에 가까울수록 변화 작아짐, 절대 안 넘음
3. vitality 업데이트 — 이벤트 피해를 확률로 깎음(고정값 아님). (선택: 노화 자연감소)
4. status 판정 — vitality ≤ 0 → TERMINATED
```
- 적응 공식 선택 이유: 자르기(clamping)는 경계에서 변화가 죽어 이벤트가 사라짐. 적응은 경계 근처에서도 계속 움직이며 범위를 자동 보장(예외 처리 불필요).

### 12.4 이벤트
- **종류는 유한**(사전에 나열 가능), 무한한 다양성은 **세부 결과를 확률**로 표현(2층 구조).
- 이벤트 사전 = **영향 숫자 테이블**(문장 아님). 예: `교통사고 → {신경증 +0.3, 외향성 -0.2, vitality -(확률)}`.
- **뽑기 2단계**: ① 좋은/나쁜(1단계 비율 = 우주 성향) → ② 해당 사전에서 세부 종류(가중치).
- 뽑기 구현: 가중치 → 누적합(prefix sum, 1회 준비) → **이진 탐색 O(log n)**. 한 tick에 이벤트 0~여러 개.
- "이유"는 로그(몇 살 몇 월 무슨 이벤트 → 어떤 값 변화)로 남김. 서사(문장)는 나중.

### 12.5 탄생 판정 (NOT_BORN)
- 예/아니오라 **확률 하나**(가중치 아님): `r = random(0,1); r < p_born → 태어남 else NOT_BORN`.
- `p_born`은 **시드에서 파생**(우주마다 다름 + 재현 가능). 우주별 탄생 난이도를 표현.
- NOT_BORN도 **집계 분모(전체 우주 수)에 포함**.

### 12.6 우주 환경 파라미터 (전부 시드에서 독립 파생)
- `p_born`(탄생 확률), 나쁜 일 비율(이벤트 1단계), 생존 난이도, 사고 확률, 이벤트 분포 등.
- **서로 독립** — 억지 상관관계 안 넣음("나쁜 일 많은데 안 죽는 우주" 같은 다양한 조합 허용).

### 12.7 몬테카를로 실행
- 우주 하나 완전체(시드 → p_born → tick 루프 → 최종 상태)를 **for문으로 N번** 감쌈.
- **시드 = 우주 번호(i 기반).** 실행 순서와 무관하게 우주 k는 항상 시드 k → 병렬 전환해도 재현 안전. 워커가 실행 중 즉석 시드 생성은 금지.
- 시드는 **정보 없는 출발점 숫자.** 환경 값은 시드에 담긴 게 아니라 PRNG로 굴려 생성. 같은 시드 = 같은 우주.
- **무한 표현 불필요**: `uint64_t`(2^64 ≈ 1,800경)면 실제 탐색(수백만)보다 압도적으로 커서 사실상 무한. **극히 일부만 뽑아 통계적 수렴으로 "무한의 답"을 추정.**
- 개체는 끝까지 흩어지고, **비율만 수렴**. N=1,000은 순차로 수 초 → 병렬은 실측 후.

### 12.8 base(기준)
- **같은 나라·같은 인종으로 고정.** 갈라짐 원인을 환경(시드)으로만 귀속 → 실험이 깨끗.
- 국적/종(種) 차이(base 자체가 다름)는 판타지 우주처럼 **별도 실험**으로 분리.

### 12.9 결과 분석
- **축별 변화**(그냥 빼기, 부호 살림) → "어떻게 변했나" 서술.
- **base와의 거리 = 유클리드 거리** → "얼마나 멀어졌나" 하나의 값.
  - 유도: 축별 차이 → 제곱(부호 제거 + 극단 강조) → 합 → 제곱근.
  - 코사인 유사도는 크기를 무시 → 우리는 정도 차이가 의미 있어 **유클리드 채택**.

### 12.10 클러스터링 & 차원 축소
- **클러스터링 = k-means**(N에 비례, 대규모 OK). k는 **엘보우/실루엣**으로 선택(유형 수 몰라도 됨).
  - 계층적 군집화는 N²이라 대규모에서 탈락. (대규모 시 표본 추출도 가능 — 통계적 수렴이 대표성 보장)
- **차원 축소 = PCA**. "버리기"(정보 손실) 아니라 "합치기"(정보 보존). **분산 큰 방향부터** 새 축으로(퍼진 방향 = 정보 많은 방향).

### 12.11 지표
- **R_exist(생존율)** = `ALIVE / (ALIVE + TERMINATED + NOT_BORN)` = ALIVE / 전체. (분모는 항상 전체)
  - 함께: 탄생 실패율 = NOT_BORN/전체, 사망률 = TERMINATED/전체 (셋 합 = 100%).
- **S_identity(base 유사도)** = `1 - (정규화된 base 거리)`. (나눗셈은 거리 0에서 터지고 범위 지저분 → 빼기 채택)

### 12.12 시각화
- **Plotly 2D 산점도**(= 분기 스펙트럼). 한눈에 보기 위해 2D.
  - 점 = 살아남은 우주(PCA로 2D 압축 좌표), 색 = k-means 유형.
  - 죽은/안 태어난 우주는 점으로 안 그림 → 개수만 숫자로 별도 표기.
- 나중 확장(1차 이후): tick별 흩어짐 애니메이션, 점 클릭 시 개별 우주 생애 로그.

### 12.13 언어 / 규모
- **1차는 Python**(NumPy·scikit-learn·Plotly) — 목적이 "결과 확인"이라 구조·분석·시각화 반복 속도가 우선.
- C++/Rust 이식은 N을 크게 키워 실측상 느릴 때만(Phase 5). base·규모·병렬 모두 "실측 후 결정".